...
public class DataUsersDao {
	...
	public void writeDates(ArrayList<User> userDataList, String languaje) {
	...
	public void writeDates(ArrayList<User> userDataList) {
		...
		data.put("userList", userList);
		data.put("languaje", "src/data/languajes/Espa\u00F1ol.properties");
		try (FileWriter file = new FileWriter("src/data/usersdata/UserData.json")) {
			file.write(data.toString());
		} catch (Exception e) {
		}
	...
	public String getLanguaje(JSONObject jsonObject) {
		return jsonObject.getString("languaje");
	}
}
